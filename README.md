...
`Badge` and `Card` are the first migrated components: `Badge.to_html()` emits pure HTML markup (`<div data-component="badge" …>`) styled by `dr.css`; `Card.to_html()` composes `HtmlComponent` children (e.g. nested badges). Plotly charts still use `.render()` / `ActiveHtml` during migration — see the foundation proof probe.
